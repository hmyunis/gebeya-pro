...
function buildInitialSlots(existingImages: ExistingProductImage[]): ImageSlot[] {
  const slots: ImageSlot[] = new Array(MAX_PRODUCT_IMAGES)
    .fill(null)
    .map(() => ({ kind: "empty" }));
  existingImages.slice(0, MAX_PRODUCT_IMAGES).forEach((image, index) => {
    slots[index] = {
      kind: "existing",
      path: image.path,
      url: image.url,
...
export default function ProductImageUploader({
  ...
  const clearAllSlots = () => {
    setSlots((currentSlots) => {
      ...
      return new Array(MAX_PRODUCT_IMAGES).fill(null).map(() => ({ kind: "empty" }));
    });
  };
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-default-400">
        Up to {MAX_PRODUCT_IMAGES} images. Fill each slot individually.
      </p>

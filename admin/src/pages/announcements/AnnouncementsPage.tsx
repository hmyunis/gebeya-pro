...
export default function AnnouncementsPage() {
  ...
  const [role, setRole] = useState<BroadcastRole>('customer');
  ...
  const runsQuery = useQuery({
    queryKey: ['broadcast', 'runs', runsPage, runsLimit],
  ...
  const createMutation = useMutation({
    ...
    onSuccess: async (result) => {
      await queryClient.invalidateQueries({ queryKey: ['broadcast', 'runs'] });
      setMessage('');
      setSelectedUsers([]);
      addToast({
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-default-500">
          Broadcast announcements, news, or ads to linked users or everyone who started the bot.
        </p>
      </div>

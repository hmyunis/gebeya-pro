...
import type { BroadcastRole, BroadcastUser } from '../types';
...
export function AudienceUserPicker({
  ...
  const { data, isFetching } = useQuery({
    queryKey: ['broadcast', 'users', trimmedSearch, role],
    queryFn: async () => listBroadcastUsers(trimmedSearch, 1, 15, role),
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-default-500">
                    @{user.username || user.loginUsername || 'no_username'} · Role{' '}
                    {user.role ?? 'customer'} · TG {user.telegramId || 'none'}
                  </p>
                </div>
                <Button
                  size="sm"

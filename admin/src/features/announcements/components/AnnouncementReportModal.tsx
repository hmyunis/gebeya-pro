...
export function AnnouncementReportModal({
  ...
  const deliveriesQuery = useQuery({
    queryKey: ['broadcast', 'run', runId, 'deliveries', filter, page, limit],
    queryFn: async () =>
  ...
  const cancelMutation = useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([
        ...
        queryClient.invalidateQueries({ queryKey: ['broadcast', 'run', runId] }),
        queryClient.invalidateQueries({
          queryKey: ['broadcast', 'run', runId, 'deliveries'],
        }),
      ]);
      addToast({
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-default-500">
              ...
              {row.original.recipient.sourceUser
                ? `Role ${row.original.recipient.role ?? 'unknown'}`
                : 'Bot subscriber'}
            </p>
          </div>
        ),
